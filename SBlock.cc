    ...
    if (this->direction == 1) {// direction 1 means the block is parallel with 1 at top left
        ...
        for (auto aCell : this->cells){
            if (i == 0 || i == 1){ // 1st, 2nd cells
                ...
                Coordinate coord{aCell->getCoord().r+(3-i), aCell->getCoord().c+(2-i)};
                futureCoord.emplace_back(coord);
            }
            i++;
        }
        ...
        for (auto aCell : this->cells){
            if (i == 0 || i == 1){ // 1st, 2nd cells
                ...
                Coordinate coord{aCell->getCoord().r-(i-1), aCell->getCoord().c-(3-i)};
                futureCoord.emplace_back(coord);
            }
            i++;
        }
...
SBlock::SBlock(int level, gameInterface *theInterface):Block(level, theInterface){
    type = 'S';
